...
var calculateScore = function(doc) {
  ...
  */
  ...
  if(doc.copyright && doc.copyright < new Date().getFullYear()) {
    doc.score -= 10;
  }
  if(doc.score < 0) {
    doc.score = 0;
...
var findCopyright = function(b) {
  var m = b.match(/copyright.+(19|20|21)[0-9][0-9]/ig);
  if(!m) {
    m = b.match(/\(c\).+(19|20|21)[0-9][0-9]/ig);
  }
  if(!m) {
    m = b.match(/&copy;.+(19|20|21)[0-9][0-9]/ig);
  }
  if(m != null) {
    var year = m[0].match(/(19|20|21)[0-9][0-9]/);
    if(year) {
      year = year[0];
...
var spider = function (url, callback) {  
  ...
  request(req , function(e, r, b) {
    if(e) {
      ...
      var $ = cheerio.load(b, {lowerCaseTags: true, lowerCaseAttributeNames: true});
      var a_tags = $('a');
      var linksonly = [];
      ...
      doc.telephone = 0;
      doc.meta_keywords = ($('meta[name="keywords"]').length > 0 || $('meta[name="KEYWORDS"]').length > 0)?1:0;
      doc.meta_description = ($('meta[name="description"]').length > 0 || $('meta[name="DESCRIPTION"]').length > 0)?1:0;
      doc.html5 = (b.indexOf("<!DOCTYPE html>") > -1 || b.indexOf("<!doctype html>") > -1)?1:0;
      doc.xhtml = (b.indexOf("xhtml") > -1 || b.indexOf("XHTML") > -1)?1:0;
      doc.html4 = (b.indexOf("html4") > -1 || b.indexOf("HTML4") > -1)?1:0;
